...
describe(__filename, () => {
  ...
  it('should render correctly', () => {
    ...
    const props = {
      registryModalData: { itemType: 'MSWP' },
  ...
  it('should not render when data is null', () => {
    const content = null;
    const tree = shallow(
      <RegistryDetailModalRecommendation labels={labels} content={content} />
    );

    expect(tree.find('Accordion')).to.have.length(0);
  });
  ...
  it('shouldComponentUpdate should return true with Date', () => {
    ...
    const tree = shallow(<RegistryDetailModalRecommendation labels={labels} />);
    expect(tree.instance().shouldComponentUpdate(nextProps)).to.equals(true);
  });
});
